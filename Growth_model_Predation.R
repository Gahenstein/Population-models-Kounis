...
pred_growth_function <- function (a = rep(20,6), b = rep(5,6), t = 1000,
  ...
  ### Dataframes with initial populations at tick 0 for 6 cohorts----
    ...
    # Initial predator resources equal to pop
    ...
    results_prey <- data.frame("tick" = 0, 
                          ...
                          # calculated as the population in a cohort multiplied by
                          # the ratio of available in cohort (prey cohort pop * avoidance) divided by
                          # base predator resource need (pred pop * mean pred_efficiency)
                          "pr1" = NA, 
                          "pr2" = NA, 
                          "pr3" = NA,
                          "pr4" = NA,
  ...
  for (i in 1:parameters_prey$t) {
    ...
    for(o in 2:6){
      results_prey[i+1, paste("C", o, sep = "")] <- do.call(Cn_growth, # Call function for age 1 growth (survivors)
                                                            list(results_prey[i,paste("C", o-1, sep = "")], # Survivng age 0
                                                                 d1[o] + d1[o] * res_dmod_prey)) # Mortality of age 1 modified by available res
      results_prey[i+1, paste("pr", o, sep = "")] <- results_pred$total[i] * results_pred$mean_ef[i] * av[o] # Calculate deaths to predation
      if (results_prey[i+1, paste("pr", o, sep = "")] > results_prey[i+1, paste("C", o, sep = "")]) { # Make sure predation deaths dont exceed cohort pop
        results_prey[i+1, paste("pr", o, sep = "")] <-  results_prey[i+1, paste("C", o, sep = "")]
        }
      results_prey[i+1, paste("C", o, sep = "")] <- results_prey[i+1, paste("C", o, sep = "")] - results_prey[i+1, paste("pr", o, sep = "")] # Update cohort pop after predation
    }
    ...
    for(p in 2:6){
    results_pred[i+1, paste("C", p, sep = "")] <- do.call(Cn_growth, # Call function for age 1 growth (survivors)
                                 list(results_pred[i, paste("C", p-1, sep = "")], # Survivng age 0
                                      d2[p] + d2[p] * res_dmod # Mortality of age 1 modified by available res
                                 )
    )
    }
  ...
  #   )
  #   for(l in 1:6){
  #   lines(results_prey[,paste("C", l, sep = "")] ~ results_prey$tick,
  #         type = "h",
  #         col = l+1)
  #     }
  #   legend("topright",
  ...
  #   )
  #   for(l in 1:6){
  #     lines(results_pred[,paste("C", l, sep = "")] ~ results_pred$tick,
  #           type = "h",
  #           col = l+1)
  #   }
  #   legend("topright",
